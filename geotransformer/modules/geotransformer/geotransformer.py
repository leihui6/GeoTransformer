...
class GeometricStructureEmbedding(nn.Module):
    ...
    def get_embedding_indices(self, points: torch.Tensor):
        ...
        dist_map = torch.sqrt(pairwise_distance(points, points))

        d_indices = dist_map / self.sigma_d

        k = self.angle_k

        topk_values, topk_indices = dist_map.topk(
            k=k + 1,
            dim=2,
            largest=False
        )
        ...
        knn_indices = knn_indices.unsqueeze(3)
        knn_indices = knn_indices.expand(
            batch_size,
            num_point,
            k,
            3
        ...
        expanded_points = expanded_points.expand(
            batch_size,
            num_point,
            num_point,
            3
        ...
        ref_vectors = ref_vectors.unsqueeze(2)
        ref_vectors = ref_vectors.expand(
            batch_size,
            num_point,
            num_point,
            k,
        ...
        anc_vectors = anc_vectors.unsqueeze(3)
        anc_vectors = anc_vectors.expand(
            batch_size,
            num_point,
            num_point,
            k,
        ...
        sin_values = torch.linalg.norm(
            torch.cross(ref_vectors, anc_vectors, dim=-1),
            dim=-1
        )
